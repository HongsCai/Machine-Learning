...
如上第三张图所示，模型在拟合训练数据上做得非常好，完美通过所有训练数据，使误差为 $0$，但是其图像表现的抖动异常，甚至预测出房子更大，其价格甚至更低。这种就叫做**过拟合** (Overfitting)，也称为**高方差** (High variance)，完美拟合训练集数据，但其泛化能力差，不能推广至不属于训练集的例子上。
